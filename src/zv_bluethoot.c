...
static const char *TAG = "ZV_BLUETHOOT";
const ble_signature_t zv_signatures[] = {
    { 0x004C, "Apple" },
    { 0x0075, "Samsung" },
    { 0x038F, "Xiaomi" },
    { 0x027D, "Huawei"}
...
void get_manufacturer_name(struct ble_scan_result_evt_param scan_result, char *out_name, int out_length)
{
    ...
    // ESP_LOGI(TAG, "company_id : %d", company_id);
    // ESP_LOGI(TAG, "manufactured: %d", adv_manufactured[1], adv_manufactured[0]);

    const char *name = "Unknown";
    for (int i = 0; i < 5; i++)
    {
        if (zv_signatures[i].company_id == company_id)
        {
            name = zv_signatures[i].name;
            break;
        }
    }
...
void get_device_name(struct ble_scan_result_evt_param scan_result, char *out_name, int out_length)
{
    ...
    // ESP_LOGI(TAG, "found name: %s", out_name);
}