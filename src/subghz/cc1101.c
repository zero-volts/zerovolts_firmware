...
static uint8_t zv_cc1101_read_register(uint8_t address, uint8_t burst)
{
    if (cc_device_handle == NULL)
        return ((uint8_t)-1);
...
uint8_t zv_cc1101_read_status_register(uint8_t address)
{
    ...
    return zv_cc1101_read_register(address, CC1101_READ_BURST);
}

uint8_t zv_cc1101_read_config_register(uint8_t address)
{
    return zv_cc1101_read_register(address, CC1101_READ_SINGLE);
}
...
esp_err_t zv_cc1101_config_and_verify()
{
    ...
    for (int i= 0; i < config_length; i++)
    {
        ...
        zv_cc1101_write_config_register(reg_value.address, reg_value.value);
        uint8_t new_value = zv_cc1101_read_config_register(reg_value.address);
        if (reg_value.value != new_value) {
...
{
    return zv_cc1101_read_status_register(CC1101_STATUS_PARTNUM);
}

uint8_t zv_cc1101_read_version(void)
{
    return zv_cc1101_read_status_register(CC1101_STATUS_VERSION);
}

int8_t zv_cc1101_get_rssi_dbm(void)
{
    uint8_t raw = zv_cc1101_read_status_register(CC1101_STATUS_RSSI);
